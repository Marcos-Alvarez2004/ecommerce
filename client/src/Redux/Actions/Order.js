...
// order action
export const orderAction = (order) => async (dispacth, getState) => {
  try {
    dispacth({ type: ORDER_REQ });
    ...
    dispacth({ type: ORDER_SUCCESS, payload: data });
    dispacth({ type: CART_ITEM_CLEAR, payload: data });
  } catch (error) {
    console.log(error);
  }
};
